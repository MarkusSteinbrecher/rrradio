...
Candidate rows carry a stamped `disposition` (imported / available / duplicate / http-only / broken / unprobed — dedupe-group aware, so any group with an imported member marks its other members duplicate; `http-only` = probe-verified alive on plain http with no https equivalent, blocked from the https app origin, while `broken` means dead or failing) and, when the playability probe ended somewhere other than the record URL, a `playableUrl` (e.g. the verified https upgrade of an http record — `playable-check` tries https first and only reports `ok` when it works; genuinely http-only streams get `broken-mixed`). For http records the probe tries two https variants — the same-port scheme swap (what browser mixed-content auto-upgrade does) and the host on default port 443 (reverse-proxy TLS, e.g. streamtheworld's `:3690`) — and upgrades recurse through redirect hops to depth 2 (radiojar's tokened 302 chains). Responses Node's strict HTTP parser rejects (bare-LF status lines — regiocast/streamabc) re-probe over a lenient raw socket, since browsers play them. `tools/import-playable-candidates.mjs` imports http records via that verified https entry point. Note `broken-mixed` means *unplayable from the https app origin* — most such streams are alive on plain http and play when opened directly.
