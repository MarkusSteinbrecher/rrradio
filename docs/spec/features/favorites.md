...
| Drag-to-reorder + delete mode (jiggle/badges) | Partial; drag-to-reorder only — no delete mode, jiggle, minus badges, or undo toast (removal is the row heart). | Reference. | Supported (2026-07-03) — long-press lift-drag reorder in every display mode (live single-column reorder; drop-commit in wide-canvas grids), minus-badge delete mode with undo Snackbar. No jiggle by product decision: Android follows its home-screen convention. |
...
| Per-row now-playing metadata (list/tiles) | Not planned for current web; now-playing metadata shows only for the active station (mini-player / Now Playing), not per favorite row. | Reference. | Supported (2026-07-04) — 60 s per-row poll while Favorites or a Library station feed (Recents / list detail) is visible, scoped to that feed's stations; the currently-playing row prefers the live player snapshot over the poll. |
