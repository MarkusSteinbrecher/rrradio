...
- **Run `git gc` from time to time.** Because the large catalog artifacts (`public/stations-ios-local.json`, `public/sources/radio-browser-candidates.json`, `public/stations.json`, …) are committed and re-churn on every build, `.git` accumulates loose objects fast — it bloated to ~1.5 GB once. When the repo feels heavy, run `git gc --prune=now` (packs loose objects, ~1 GB reclaimed last time) and `rm -rf dist` (gitignored build output, regenerated by `npm run build`).
